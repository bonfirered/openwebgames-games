...
// performance.now might get faked, so save it for cpu profiler.
...
var emscriptenCpuProfiler = {
	...
	// Carries # samples equal to the pixel width of the profiler display
	// window, and old samples are erased in a rolling window fashion.
	timeSpentInMainloop: [],

	// Similar to 'timeSpentInMainloop', except this stores msec durations
	// outside the emscripten main loop callback. (either idle CPU time,
	// browser processing, or something else)
	timeSpentOutsideMainloop: [],

	// Specifies the sample coordinate into the timeSpentIn/OutsideMainloop
	// arrays that is currently being populated.
	currentHistogramX: 0,
	...
	// The 2D canvas DOM element to which the CPU profiler graph is rendered
	// to.
	canvas: null,
	...
	// History of wallclock times of N most recent frame times. Used to
	...
	// fpsCounter() is called once per frame to record an executed frame time,
	// and to periodically update the FPS counter display.
	fpsCounter: function fpsCounter(){
		...
		if (this.fpsCounterTicks.length < 120){
			this.fpsCounterTicks.push(now);
		} else {
			for (i = 0; i < this.fpsCounterTicks.length-1; ++i){
				this.fpsCounterTicks[i] = this.fpsCounterTicks[i+1];
			}
		...
		if (now - this.fpsCounterLastPrint > 2000){
			...
			var maxDt = 0;
			var nSamples = 0;
			for (i = 0; i < this.timeSpentInMainloop.length; ++i){
				dt = this.timeSpentInMainloop[i] + this.timeSpentOutsideMainloop[i];
			...
			var dtVariance = 0;
			for (i = 1; i < this.timeSpentInMainloop.length; ++i){
				dt = this.timeSpentInMainloop[i] + this.timeSpentOutsideMainloop[i];
				var d = dt - avgDt;
				dtVariance += d*d;
			}
			...
			var str = 'Last FPS: ' + fps.toFixed(2) + ', avg FPS:' + avgFps.toFixed(2) + ', min/avg/max dt: ' + minDt.toFixed(2) + '/' + avgDt.toFixed(2) + '/' + maxDt.toFixed(2) + ' msecs, dt variance: ' + dtVariance.toFixed(3) + ', JavaScript CPU load: ' + asmJSLoad.toFixed(2) + '%';
	...
	enterSection: function enterSection(sectionNumber){
		...
		++sect.count;
		if (sect.count == 1){
			sect.startTick = performance.realNow();
		}
	},

	// Call at runtime when the code execution exits the given profiling
	// section. Be sure to match each startSection(x) call with a call to
	// endSection(x).
	endSection: function endSection(sectionNumber){
		var sect = this.sections[sectionNumber];
		--sect.count;
		if (sect.count === 0){
			sect.accumulatedTime += performance.realNow() - sect.startTick;
		}
	},
	...
	frameEnd: function frameEnd(){
		// Aggregate total times spent in each section to memory store to wait
		// until the next stats UI redraw period.
	...
	initialize: function initialize(){
		// Create the UI display if it doesn't yet exist. If you want to
		// customize the location/style of the cpuprofiler UI, you can manually
		...
		this.updateUi();
		Module.preMainLoop = function cpuprofiler_frameStart(){
			emscriptenCpuProfiler.frameStart();
		};
		Module.postMainLoop = function cpuprofiler_frameEnd(){
	...
	drawBar: function drawBar(x){
		...
		// Be very lax, allow 1msec extra jitter.
		...
		if (y < fps30Limit){
			...
			this.drawContext.fillStyle = "#60A060";
		}
		this.drawContext.fillRect(x, y, 1, h);
	},

	// Main UI update/redraw entry point. Drawing occurs incrementally to touch
	// as few pixels as possible and to cause the least impact to the overall
	...
	updateUi: function updateUi(startX, endX){
		...
		if (this.canvas.width != document.documentElement.clientWidth - 32){
			this.canvas.width = document.documentElement.clientWidth - 32;
			if (this.timeSpentInMainloop.length > this.canvas.width){
				this.timeSpentInMainloop.length = this.canvas.width;
			}
			if (this.timeSpentOutsideMainloop.length > this.canvas.width){
				this.timeSpentOutsideMainloop.length = this.canvas.width;
			}
			if (this.lastUiUpdateEndX >= this.canvas.width){
				this.lastUiUpdateEndX = 0;
			}
			if (this.currentHistogramX >= this.canvas.width){
				this.currentHistogramX = 0;
			}
			for (var i in this.sections){
				var sect = this.sections[i];
				if (sect.frametimes.length > this.canvas.width){
					sect.frametimes.length = this.canvas.width;
				}
			}
			this.clearUi(0, this.canvas.width);
			this.drawGraphLabels();

			// Full redraw all columns.
			startX = 0;
		}
		...
		if (endX < startX){
			for (x = startX; x < this.canvas.width; ++x){
				this.drawBar(x);
			}
			startX = 0;
		}
